...
static IVFDataForMerge decode_concat_to_ivf_data(
    ...
    const bool all_have_stored_t0 = keep_old_state &&
...
static void encode_old_state_k0_code(
    ...
    if (nb_bits > 1 && tpred > 0.0f && adaptive_t) {
        ...
        auto evaluate_scale = [&](int step, float sign) {
            ...
            make_tpred_full_code(new_residual.data(), d, nb_bits, tpred * scale, candidate_code);
            const float cand_j = old_state_objective(candidate_code, new_residual.data(), d, nb_bits);
            if (cand_j > best_j) { best_j = cand_j; best_code = candidate_code; }
            return cand_j;
        };
